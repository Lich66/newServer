...
export class ClubUser {
    public static async findClubUser(json: IClubUserRequest): Promise<tbl_clubuser> {
        let result = await tbl_clubuser.findOrCreate({ where: { clubid: json.clubid, roomid: json.userid, cid: { [Op.regexp]: '\.' } } });
        return result[0];
    }
    public static async deleteClubUser(json: IClubUserRequest): Promise<number> {
        return await tbl_clubuser.destroy({ where: { clubid: json.clubid, roomid: json.userid, cid: { [Op.regexp]: '\.' } } });
    }
